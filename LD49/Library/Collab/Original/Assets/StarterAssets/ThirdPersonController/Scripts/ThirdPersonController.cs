...
namespace StarterAssets
{
	...
	public class ThirdPersonController : MonoBehaviour
	{
		...
		private void UpdateTray()
		{
			...
			float minGoalDist = 5.0f;

			if(distToGoal < minGoalDist)
			{
				float ratio = distToGoal / minGoalDist;
				float flex = Math.Abs(_TrayEndHeight - _TrayStartHeight) * ratio;
				//var matrix = transform.localToWorldMatrix;
				//Vector4 inFront = new Vector4(0.0f, 0.0f, ratio, 0.0f);
				//inFront = matrix * inFront;
				//_leftTargetTrans.position = new Vector3(inFront.x + _leftTargetTrans.position.x, _TrayStartHeight +  flex, inFront.z + _leftTargetTrans.position.z);
				//_rightTargetTrans.position = new Vector3(inFront.x + _rightTargetTrans.position.x, _TrayStartHeight + flex, inFront.z + _rightTargetTrans.position.z);
